...
Open-model `ramping-arrival-rate`: requests are started at a target rate independent of server latency, ramping 0 -> `RATE/RAMP_STEPS` -> … -> `RATE` req/s, holding `STAGE_TIME` at each plateau with `RAMP_TIME` ramps in between, then back to 0. If you see `dropped_iterations` in the summary, the `MAX_VUS` pool was exhausted: the server can't sustain the rate at its current latency, so latencies at that plateau are queue-dominated (read the raw per-second curve, not the run average).
