...
import io.gravitee.singularitee.engine.api.tools.ToolCallExtractor.ExtractedToolCall;
...
class ToolCallExtractorTest {
  ...
  private static List<ExtractedToolCall> extract(String output) {
    return ToolCallExtractor.extract(output, List.of(), null, RENDERER);
  }
  ...
  class ChatmlJson {
    ...
    void garbageYieldsNoCalls() {
      ...
      assertThat(ToolCallExtractor.extract(null, List.of(), null, RENDERER)).isEmpty();
    }
  }
  ...
  class Config {
    ...
    void namedBuiltinRestrictsToThatDialect() {
      ...
      assertThat(ToolCallExtractor.extract(gemma, List.of(), "gemma-call", RENDERER)).hasSize(1);
      assertThat(ToolCallExtractor.extract(gemma, List.of(), "chatml-json", RENDERER)).isEmpty();
    }
    ...
    void brokenTemplateFailsOpen() {
      assertThat(
        ToolCallExtractor.extract("anything", List.of(), "{% for x in %}broken", RENDERER)
      ).isEmpty();
      assertThat(
        ToolCallExtractor.extract("anything", List.of(), "this renders no json", RENDERER)
      ).isEmpty();
    }
    ...
    void truncatedBareSpanIsBalancedAndRecovered() {
      ...
      var calls = ToolCallExtractor.extract(truncated, List.of(), "chatml-json", RENDERER);
      assertThat(calls).hasSize(1);
    ...
    void extractResultSurfacesFailureCause() {
      var broken = ToolCallExtractor.extractResult(
        "anything",
        List.of(),
        "{% for x in %}broken",
        RENDERER
      ...
      var notJson = ToolCallExtractor.extractResult(
        "anything",
        List.of(),
        "this renders no json",
        RENDERER
      );
      ...
      var noCall = ToolCallExtractor.extractResult("plain prose", List.of(), null, RENDERER);
    ...
    void toolsVariableIsAvailableToTemplates() {
      ...
      var calls = ToolCallExtractor.extract(
        "x",
        List.of(java.util.Map.of("name", "f")),
        template,
        RENDERER
      );
      assertThat(calls).hasSize(1);
  ...
  class GlmNameJson {
    ...
    private static List<ExtractedToolCall> glm(String output) {
      return ToolCallExtractor.extract(output, TOOLS, "glm-name-json", RENDERER);
    }
