...
class DelegatingChatTemplateRendererTest {
  ...
  private DelegatingChatTemplateRenderer renderer;

  @BeforeEach
  void setUp() {
    renderer = new DelegatingChatTemplateRenderer(new JinjaTemplateRenderer());
  }
  ...
  class Qwen3ThinkingBypass {
    ...
    private static String qwen3Template() {
      try (
        var in = DelegatingChatTemplateRendererTest.class.getResourceAsStream(
    ...
    void string_false_does_NOT_trigger_prefill() {
      ...
      // Jinja. A quoted YAML value ("false") arrives as a String and
      // silently disables the bypass; this pins down that semantic so the
      // hardening warning in TemplateContextHelper stays honest.
