 ...
 *
 * <p>The constants are the CANONICAL names; the names the model and the wire
 * see are the deployment's {@link ServerToolNames}, which default to the
 * canonical ones. Renaming is presentation only: internal identity, stored
 * state and the proto contract never change with it.
 */
public final class TodoTools {
  ...
  /** Canonical names of the tools the server owns. */
  ...
  /** The tool definitions to register on the pipeline context, under the given names. */
  public static List<ToolDefinition> definitions(ServerToolNames names) {
    return List.of(
      ToolDefinition.newBuilder()
        .setName(names.display(SET_TODOS))
        .setDescription(
        ...
        .build(),
      ToolDefinition.newBuilder()
        .setName(names.display(COMPLETE_TODO))
        .setDescription(
        ...
        .build(),
      ToolDefinition.newBuilder()
        .setName(names.display(ASK_USER))
        .setDescription(
        ...
        .addParameters(
          ToolParameterDef.newBuilder()
            ...
            .setType("array")
            .setDescription(
              "Closed answer choices when the answers are enumerable (e.g. the branches of " +
                "an ambiguity, as strings). Prefer them: a closed question is answered in " +
                "seconds."
            )
        )
        .build()
