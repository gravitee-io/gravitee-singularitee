...
/** Perplexity over the last min(8, n) tokens: the commit-point / conclusion confidence. */
public final class TailPerplexity implements ConfidenceSignal {

  public String field() {
    return "tail_perplexity";
  }

  public double compute(TokenConfidence c) {
    var xs = c.chosen();
    if (xs.isEmpty()) return 0.0;
    int k = Math.min(8, xs.size());
    return ConfidenceSignal.perplexity(xs.subList(xs.size() - k, xs.size()));
  }
}
