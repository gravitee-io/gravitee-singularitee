 ...
 */
package io.gravitee.singularitee.engine.api.pipeline;
...
public final class PipelineContext {
  ...
  /** The display names of the server-owned tools; the executor sets the deployment's. */
  private volatile ServerToolNames serverToolNames = ServerToolNames.DEFAULTS;
   ...
   * a loop step's {@code retry_sampling_params}) and cleared when the loop exits (happy
   ...
   * list (see {@code TemplateContextHelper}).
   */
  ...
  /** The display names of the server-owned tools for this run; never null. */
  public ServerToolNames serverToolNames() {
    return serverToolNames;
  }

  /** Sets the deployment's server-tool display names; {@code null} restores the defaults. */
  public void setServerToolNames(ServerToolNames names) {
    this.serverToolNames = names == null ? ServerToolNames.DEFAULTS : names;
  }
