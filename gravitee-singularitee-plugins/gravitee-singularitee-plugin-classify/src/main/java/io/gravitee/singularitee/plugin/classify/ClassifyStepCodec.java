...
/**
 * Parses the {@code config:} block of a {@code classify} step.
 *
 * @author Rémi SULTAN (remi.sultan at graviteesource.com)
 * @author GraviteeSource Team
 */
public final class ClassifyStepCodec implements StepConfigCodec<ClassifyStepConfig> {

  private static final ObjectMapper MAPPER = new ObjectMapper();

  @Override
  public ClassifyStepConfig parse(String stepId, Map<String, Object> raw, StepCodecContext ctx) {
    Yaml d;
    try {
      d = MAPPER.convertValue(raw, Yaml.class);
    } catch (IllegalArgumentException e) {
      throw new IllegalArgumentException(
        "Step '" + stepId + "': invalid classify config: " + e.getMessage(),
        e
      );
    }
    return new ClassifyStepConfig(d.modelId(), d.inputField(), d.outputField(), d.threshold());
  }
