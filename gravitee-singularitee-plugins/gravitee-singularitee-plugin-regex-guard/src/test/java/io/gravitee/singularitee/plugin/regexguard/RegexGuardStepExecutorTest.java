...
class RegexGuardStepExecutorTest {
  ...
  @BeforeEach
  void setUp() {
    executor = new RegexGuardStepExecutor(new JinjaTemplateRenderer());
  }
  ...
  private static RegexEntity entry(String name, String pattern) {
    return new RegexEntity(name, pattern);
  }

  private static Builder cfg(GuardAction action) {
    return new Builder(action);
  }

  /** Mirrors the proto builder the original test used, over the plugin record. */
  private static final class Builder {

    private final GuardAction action;
    private final List<RegexEntity> patterns = new java.util.ArrayList<>();
    private boolean redactWithEntityType;
    private String message;

    Builder(GuardAction action) {
      this.action = action;
    }

    Builder addPatterns(RegexEntity e) {
      patterns.add(e);
      return this;
    }

    Builder setRedactWithEntityType(boolean v) {
      redactWithEntityType = v;
      return this;
    }
    ...
    RegexGuardStepConfig build() {
      return new RegexGuardStepConfig(null, patterns, redactWithEntityType, action, null, message);
    }
  }
  ...
  void noPatterns_skipsStep() {
    ...
    var cfg = cfg(GuardAction.REJECT).build();
  ...
  class TriggerMode {
    ...
    void noMatch_passesThrough() {
      ...
      var ctx = stepContext(pctx);
      var cfg = cfg(GuardAction.REJECT)
    ...
    void warnAction_doesNotHalt() {
      ...
      var ctx = stepContext(pctx);
      var cfg = cfg(GuardAction.WARN).addPatterns(entry("SSN", "\\d{3}-\\d{2}-\\d{4}")).build();
  ...
  class RedactMode {
    ...
    void noMatch_writesOriginalToOutputField() {
      ...
      var ctx = stepContext(pctx);
      var cfg = cfg(GuardAction.REDACT)
