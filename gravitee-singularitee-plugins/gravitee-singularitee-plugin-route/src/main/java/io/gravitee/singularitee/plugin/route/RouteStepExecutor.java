 ...
 *
 * <p>For {@link RoutingStrategy#EMBEDDING_KNN}, reference embeddings
 * are pre-computed at workspace load time via {@link #rxWarmupEmbeddings(PipelineModel)}
 * and cached so that runtime routing only requires a single embedding call.
 *
 * @author Rémi SULTAN (remi.sultan at graviteesource.com)
 * @author GraviteeSource Team
 */
public final class RouteStepExecutor implements StepExecutor<RouteStepConfig>, WarmupAware {
   ...
   */
  @Override
  public Completable rxWarmup(PipelineModel pipeline) {
    return rxWarmupEmbeddings(pipeline);
  }
  ...
  public Completable rxWarmupEmbeddings(PipelineModel pipeline) {
    ...
    for (var step : pipeline.steps()) {
      if (!StepTypes.ROUTE.equals(step.type())) continue;
      if (!(step.config() instanceof RouteStepConfig cfg)) continue;
      if (cfg.strategy() != RoutingStrategy.EMBEDDING_KNN) continue;

      var entryOpt = execContext.lookupModel(cfg.modelId());
      if (entryOpt.isEmpty()) {
        LOGGER.warn(
      ...
      String cacheKey = cacheKey(pipeline.id(), step.id());
      warmups.add(
    ...
    return rxResolveRouteLabel(cfg, text, stepId, ctx).flatMapMaybe(resolvedLabel -> {
      ...
      for (RouteRule rule : cfg.rules()) {
        if (rule.label().equals(resolvedLabel)) {
          ...
          return Maybe.just(rule.nextStepId());
        }
      }
  ...
  private Single<String> rxResolveRouteLabel(
    ...
    return switch (cfg.strategy()) {
      case CLASSIFIER -> rxClassifierRoute(cfg, text, stepId);
      case EMBEDDING_KNN -> rxEmbeddingKnnRoute(cfg, text, stepId, ctx);
      case LLM_STRUCTURED -> rxLlmStructuredRoute(cfg, text, stepId);
    };
  }
  ...
  private Single<String> rxClassifierRoute(RouteStepConfig cfg, String text, String stepId) {
    var entryOpt = execContext.lookupModel(cfg.modelId());
    if (entryOpt.isEmpty()) {
      LOGGER.warn("RouteStep '{}': classifier model '{}' not found", stepId, cfg.modelId());
      return Single.just("");
    }
  ...
  private Single<String> rxLlmStructuredRoute(RouteStepConfig cfg, String text, String stepId) {
    ...
    for (RouteRule rule : cfg.rules()) {
      if (normalized.contains(rule.label().toLowerCase(Locale.ROOT))) {
        LOGGER.debug(
  ...
  private Single<String> rxEmbeddingKnnRoute(
    ...
    var entryOpt = execContext.lookupModel(cfg.modelId());
    if (entryOpt.isEmpty()) {
      LOGGER.warn("RouteStep '{}': embedding model '{}' not found", stepId, cfg.modelId());
      return Single.just("");
    }
    if (!(entryOpt.get().engine() instanceof EmbeddingEngine ee)) {
      execContext.logTypeError(
    ...
    String cacheKey = ctx.pipeline() != null ? cacheKey(ctx.pipeline().id(), stepId) : stepId;
  ...
  private Single<List<RuleEmbedding>> rxComputeRuleEmbeddings(
    ...
    for (var rule : cfg.rules()) {
      List<String> sentences = rule.sentences();
      List<String> texts = sentences.isEmpty() ? List.of(rule.label()) : sentences;
      for (String t : texts) {
