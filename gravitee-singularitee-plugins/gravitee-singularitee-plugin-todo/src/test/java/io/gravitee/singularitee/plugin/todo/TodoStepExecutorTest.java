...
class TodoStepExecutorTest {
  ...
  private static TodoStepConfig config() {
    return new TodoStepConfig("work");
  }
  ...
  @Test
  void set_todos_result_echoes_the_compact_plan() {
    var pctx = pctx();
    pctx.setExtractedToolCalls(
      List.of(
        call(
    ...
    // The TOOL result the model reads carries the live plan: item 1 is promoted to in_progress.
    String result = pctx.messages().get(turnsBefore + 1).content();
    assertThat(result).contains("\"plan\":").contains("1. [>] a").contains("2. [ ] b");
  }

  @Test
  void complete_todo_result_echoes_the_updated_plan() {
    var pctx = pctx();
    pctx.setTodos(
      List.of(
    ...
    pctx.setExtractedToolCalls(List.of(call("complete_todo", "{\"id\":\"1\",\"note\":\"did a\"}")));
    int turnsBefore = pctx.messages().size();

    executor.execute("track", config(), stepContext(pctx)).blockingGet();

    String result = pctx.messages().get(turnsBefore + 1).content();
    assertThat(result).contains("1. [x] a").contains("2. [>] b");
  }

  @Test
  void complete_todo_without_a_note_is_refused_when_no_proof_exists() {
    var pctx = pctx();
    ...
    // The item keeps its state and the model is told to supply the result.
    assertThat(pctx.todos().get(0).status()).isEqualTo(TodoStatus.IN_PROGRESS);
    assertThat(pctx.messages().get(turnsBefore + 1).content()).contains("in 'note'");
  }

  @Test
  void complete_todo_without_note_is_allowed_when_proof_already_recorded() {
    var pctx = pctx();
    // e.g. the item was answered on a prior turn: its proof is already present.
    pctx.setTodos(
      List.of(new PipelineContext.TodoItem("1", "a", TodoStatus.IN_PROGRESS, "already answered"))
    );
    pctx.setExtractedToolCalls(List.of(call("complete_todo", "{\"id\":\"1\"}")));
  ...
  @Test
  void set_todos_forces_pending_and_ignores_an_incoming_done_status() {
    var pctx = pctx();
    pctx.setExtractedToolCalls(
      List.of(
        call(
          "set_todos",
          "{\"todos\":[{\"id\":\"1\",\"title\":\"a\",\"status\":\"done\"},{\"id\":\"2\",\"title\":\"b\"}]}"
        )
      )
    );
    ...
    // A fresh install cannot pre-complete work: item 1 starts pending (then promoted to
    // in_progress as the first item), not done, so all the work still runs.
    assertThat(pctx.todos().get(0).status()).isEqualTo(TodoStatus.IN_PROGRESS);
    assertThat(pctx.get("todos.remaining")).isEqualTo("2");
  }

  @Test
  void set_todos_rejects_duplicate_ids() {
    var pctx = pctx();
    pctx.setExtractedToolCalls(
      List.of(
        call(
          "set_todos",
          "{\"todos\":[{\"id\":\"1\",\"title\":\"a\"},{\"id\":\"1\",\"title\":\"b\"}]}"
        )
      )
    );
    int turnsBefore = pctx.messages().size();
    ...
    assertThat(pctx.messages().get(turnsBefore + 1).content()).contains("duplicate todo id '1'");
  }

  @Test
  void render_plan_markers_and_size_cap() {
    assertThat(TodoStepExecutor.renderPlan(List.of())).isEqualTo("(no plan)");

    var mixed = List.of(
      new PipelineContext.TodoItem("1", "done it", TodoStatus.DONE, null),
      new PipelineContext.TodoItem("2", "doing it", TodoStatus.IN_PROGRESS, null),
      new PipelineContext.TodoItem("3", "todo it", TodoStatus.PENDING, null)
    );
    assertThat(TodoStepExecutor.renderPlan(mixed)).isEqualTo(
      "1. [x] done it\n2. [>] doing it\n3. [ ] todo it"
    );

    // Beyond the cap, done items are tallied instead of listed line by line.
    var big = new java.util.ArrayList<PipelineContext.TodoItem>();
    for (int i = 0; i < 45; i++) {
      big.add(new PipelineContext.TodoItem("d" + i, "old " + i, TodoStatus.DONE, null));
    }
    big.add(new PipelineContext.TodoItem("p", "pending one", TodoStatus.PENDING, null));
    String rendered = TodoStepExecutor.renderPlan(big);
    assertThat(rendered).contains("1. [ ] pending one").contains("… (+45 done)");
  ...
  void jinja_constraints_variable_is_bound_and_empty_when_unset() {
    var pctx = pctx();
    assertThat(TemplateContextHelper.buildBaseContext(pctx).get("constraints")).isEqualTo("");
    pctx.setTodoConstraints("use uv");
    assertThat(TemplateContextHelper.buildBaseContext(pctx).get("constraints")).isEqualTo("use uv");
  }
  ...
  void ask_user_saves_plan_to_session_store() {
    var store = new io.gravitee.singularitee.engine.api.pipeline.TodoSessionStore(
  ...
  void empty_set_todos_is_refused_and_installs_no_plan() {
    ...
    var toolTurn = pctx
      .messages()
      .stream()
      .filter(t -> t.role() == io.gravitee.singularitee.engine.api.ChatRole.TOOL)
      .reduce((a, b) -> b)
      .orElseThrow();
