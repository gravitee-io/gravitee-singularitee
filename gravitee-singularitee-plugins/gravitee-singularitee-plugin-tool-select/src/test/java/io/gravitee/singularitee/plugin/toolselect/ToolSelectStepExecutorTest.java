...
import io.gravitee.singularitee.engine.api.ClassifierEngine.ClassifyLabel;
 ...
 * error, default label condensing and last-user-message input,.
 */
class ToolSelectStepExecutorTest {
  ...
  @BeforeEach
  void setUp() {
    executor = new ToolSelectStepExecutor(
      mock(StepExecutionContext.class),
      new JinjaTemplateRenderer()
    );
    engine = mock(ClassifierEngine.class);
  }

  /** Mutable test-side builder over the config record. */
  ...
  private static final class Cfg {
    ...
    private final List<String> alwaysInclude = new ArrayList<>();
    private boolean trimDescriptions;
    private String descriptionTemplate;

    Cfg modelId(String v) {
      modelId = v;
      return this;
    }

    Cfg inputField(String v) {
      inputField = v;
      return this;
    }
    ...
    Cfg labelTemplate(String v) {
      labelTemplate = v;
      return this;
    }

    Cfg alwaysInclude(String v) {
      alwaysInclude.add(v);
      return this;
    }

    Cfg trimDescriptions(boolean v) {
      trimDescriptions = v;
      return this;
    }

    Cfg descriptionTemplate(String v) {
      descriptionTemplate = v;
      return this;
    }

    ToolSelectStepConfig build() {
  ...
  void nine_tools_yield_three_calls_of_at_most_five_labels_including_none() {
    ...
    var cfg = cfg().modelId("triage").batchSize(4).build();
  ...
  void tool_selected_only_when_score_meets_threshold_and_beats_none() {
    ...
    var cfg = cfg().modelId("triage").threshold(0.3f).build();
  ...
  void all_none_yields_empty_shortlist_and_always_include_is_not_added() {
    ...
    var cfg = cfg().modelId("triage").alwaysInclude("tool0").build();
  ...
  void always_include_is_unioned_into_a_non_empty_shortlist() {
    ...
    var cfg = cfg().modelId("triage").alwaysInclude("tool2").alwaysInclude("not-a-tool").build();
  ...
  void label_template_renders_condensed_description() {
    ...
    var cfg = cfg()
      .modelId("triage")
      .labelTemplate("Tool {{ tool.name }}: {{ tool.description }}")
      .build();
  ...
  void input_is_last_user_message_truncated_to_1500_chars() {
    ...
    var cfg = cfg().modelId("triage").build();
  ...
  void input_field_overrides_last_user_message() {
    ...
    var cfg = cfg().modelId("triage").inputField("custom.field").build();
  ...
  void trim_descriptions_populates_condensed_map_for_selected_tools_only() {
    ...
    var cfg = cfg().modelId("triage").trimDescriptions(true).build();
  ...
  void description_template_overrides_default_condenser() {
    ...
    var cfg = cfg()
      ...
      .descriptionTemplate("{{ tool.name }} - {{ tool.description | upper }}")
      .build();
