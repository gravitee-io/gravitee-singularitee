...
public final class ToolSelectStepExecutor
  ...
  public ToolSelectStepExecutor(StepExecutionContext execContext, TemplateRenderer jinjaRenderer) {
    super(execContext);
  ...
  protected Maybe<String> rxExecuteWithEngine(
    ...
    int batchSize = cfg.batchSize() > 0 ? cfg.batchSize() : DEFAULT_BATCH_SIZE;
    float threshold = cfg.threshold() > 0 ? cfg.threshold() : DEFAULT_THRESHOLD;
    ...
    return Flowable.fromIterable(batches)
      ...
      .flatMapMaybe(perBatch -> {
        ...
        if (!shortlist.isEmpty()) {
          for (String name : cfg.alwaysInclude()) {
  ...
  private static String resolveInput(ToolSelectStepConfig cfg, PipelineContext pctx) {
    if (!cfg.inputField().isBlank()) {
      return pctx.get(cfg.inputField());
    }
   ...
   * either through the configured Jinja {@code label_template} (context:
   ...
   * {@code description_template} (Jinja, context {@code tool} map with
   ...
   * configured Jinja {@code description_template} (context: {@code tool} map
  ...
  String condenseForInjection(ToolSelectStepConfig cfg, ToolDefinition tool) {
    if (!cfg.descriptionTemplate().isBlank()) {
      try {
        return jinjaRenderer
          .render(
