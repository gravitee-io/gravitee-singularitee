...
/**
 * Creates an ONNX BERT embedding engine from a {@link ModelLoadRequest}.
 *
 ...
 *
 * <p>This class and {@link OnnxEmbeddingEngine} are the <strong>only</strong>
 * files permitted to import {@code gravitee-inference-onnx} embedding types.
 *
 * @author Rémi SULTAN (remi.sultan at graviteesource.com)
 * @author GraviteeSource Team
 */
public final class OnnxEmbeddingFactory implements ModelEngineFactory {

  private final GioMaths gioMaths;
  private final Vertx vertx;

  public OnnxEmbeddingFactory(GioMaths gioMaths, Vertx vertx) {
  ...
  public ModelEngine create(ModelLoadRequest request) throws Exception {
    ...
    if (!cfg.poolingMode().isEmpty()) onnxConfig.put(
      "poolingMode",
      resolvePoolingMode(cfg.poolingMode())
    );
    onnxConfig.put("normalize", cfg.normalize());

    var bertConfig = new OnnxBertConfig(resource, gioMaths, onnxConfig);
    return new OnnxEmbeddingEngine(new OnnxBertEmbeddingModel(bertConfig), vertx);
  }

  private static PoolingMode resolvePoolingMode(String value) {
    if (value == null || value.isBlank()) return PoolingMode.MEAN;
    try {
      return PoolingMode.valueOf(value.toUpperCase());
