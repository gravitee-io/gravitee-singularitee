 ...
 */
public final class OnnxClassifierFactory implements ModelEngineFactory {
  ...
  public ModelEngine create(ModelLoadRequest request) throws Exception {
    ...
    ClassifierMode mode = resolveClassifierMode(cfg.classifierMode());

    Map<String, Object> onnxConfig = new HashMap<>();
    if (!cfg.labels().isEmpty()) onnxConfig.put("labels", cfg.labels());
    if (cfg.maxSequenceLength() > 0) onnxConfig.put("maxSequenceLength", cfg.maxSequenceLength());
    if (!cfg.classifierMode().isEmpty()) onnxConfig.put("mode", mode);

    var bertConfig = new OnnxBertConfig(resource, gioMaths, onnxConfig);
    return new OnnxClassifierEngine(new OnnxBertClassifierModel(bertConfig), mode, vertx);
  }

  private static ClassifierMode resolveClassifierMode(String value) {
    if (value == null || value.isBlank()) return ClassifierMode.SEQUENCE;
    try {
      return ClassifierMode.valueOf(value.toUpperCase());
    } catch (IllegalArgumentException e) {
      return ClassifierMode.SEQUENCE;
    }
  }
}
