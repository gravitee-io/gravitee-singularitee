...
/**
 * BERT sentence embedder over ONNX Runtime (e.g. MiniLM, BGE).
 *
 * <p>Pools the last hidden state per {@code POOLING_MODE} ({@code CLS} row or {@code MEAN} over
 * the unpadded positions). An input longer than the sequence budget is split, each chunk
 * embedded in one batched pass, and the chunk vectors averaged by content-token count and
 * L2-normalized. Result vectors of a single chunk are the raw pooled output.
 *
...
public class OnnxBertEmbeddingModel extends OnnxBertInference<String, EmbeddingTokenCount> {
  ...
  public EmbeddingTokenCount infer(String input) {
    ...
    // Reports the full-input token count (special tokens included), not combine()'s
    // content-token sum.
    return new EmbeddingTokenCount(combine(pooled).embedding(), tokens.size());
  }

  /**
   * Full-input token count (single tokenize pass, special tokens included), the count
   * {@link #infer(String)} reports. Exposed so batched callers that recompose
   * split, encodePooled and combine themselves can report the same count.
   */
   ...
   * into a single GPU run; {@link #infer(String)} recomposes split, encodePooled and combine.
   */
  public List<ChunkEmbedding> encodePooled(List<String> chunkTexts) {
    ...
    try (var result = encodingResults.result()) {
      // One bulk copy; only each chunk's unpadded rows are copied out below.
